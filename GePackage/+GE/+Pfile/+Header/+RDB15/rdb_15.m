function strct = ge_read_rdb_header_rec_rdb15(pfile_name,offset)
...
strct = setfield(strct, 'obsolete1', fread(fid,16,'float32','ieee-le')); % PS mean noise each receiver   +-3.40282e38 
strct = setfield(strct, 'obsolete2', fread(fid,16,'float32','ieee-le')); % PS noise calc for muti rec  	+-3.40282e38 
...
strct = setfield(strct, 'rdb_hdr_off_ps', fread(fid,1,'int32','ieee-le')); % Byte offset to start of rdb_hdr_ps of POOL_HEADER            
...
strct = setfield(strct, 'rdb_hdr_swiftenable', fread(fid,1,'int32','ieee-le')); % SWIFT enable/disable flag 
strct = setfield(strct, 'rdb_hdr_numslabs', fread(fid,1,'int16','ieee-le')); % Number of slabs to be used by TRICKS 
strct = setfield(strct, 'rdb_hdr_swiftcoilnos', fread(fid,1,'uint16','ieee-le')); % Number of coils to SWIFT between 
...
strct = setfield(strct, 'rdb_hdr_dynaplan_numphases', fread(fid,1,'int32','ieee-le')); % Number of phases for Dynamic Plan 

strct = setfield(strct, 'rdb_hdr_excess', fread(fid,216,'int16','ieee-le')); % free space for later expansion 
